  ...
  object ReifiedRelation {
    ...
    private[monofunctor] class Zipped[
      ...
      override def apply(in: LeftIn): Access[Zipped] =
        foreach(
          List[Access[Any]](
            left.apply(in).asInstanceOf[Access[Any]],
            right.apply(ev(in)).asInstanceOf[Access[Any]]
          )
        )(identity).map {
      ...
      override def applyMultiple[Coll[+T] <: Iterable[T] & IterableOps[T, Coll, Coll[T]]](
        ...
        foreach(
          List[Access[Coll[Any]]](
            left.applyMultiple(in).asInstanceOf[Access[Coll[Any]]],
            right.applyMultiple(ev.liftCo(in)).asInstanceOf[Access[Coll[Any]]]
          )
        )(identity).map {
